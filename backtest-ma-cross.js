...
import MovingAverageCrossStrategy from '../src/strategies/movingAverageCross.js';
...
async function runMACrossBacktest() {
  try {
    console.log('Running Moving Average Crossover Backtest');
    ...
    const shortPeriod = parseInt(process.argv[5]) || 10;
    const longPeriod = parseInt(process.argv[6]) || 50;
    
    console.log(`Parameters:`);
    console.log(`- Ticker: ${ticker}`);
    console.log(`- Date Range: ${startDate} to ${endDate}`);
    console.log(`- Short MA Period: ${shortPeriod}`);
    console.log(`- Long MA Period: ${longPeriod}`);
...
if (process.argv[1].includes('backtest-ma-cross.js')) {
  runMACrossBacktest()
    .then(() => console.log('\nBacktest completed successfully'))
    .catch(err => console.error('\nBacktest failed:', err));
}

export default runMACrossBacktest;
